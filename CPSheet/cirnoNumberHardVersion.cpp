...
void solve(){
    ...
    for(int i = 0; i < l ; i++){
        ...
        // none of the digits d1 and d2  d10 match with the digit[i ] : then we just retrun 
        bool isNotSame = 1;
        for(auto x : d){

            if(x == dig){
                // any 1 becomes same : we can conotnue : else we can break;
                isNotSame = 0;
                break;
            }
        }
