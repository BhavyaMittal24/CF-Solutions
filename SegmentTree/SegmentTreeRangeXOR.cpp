...
class Update{
    ...
    // at leaft : a[i] =  gcd ( a[i] , val ) 
    void apply(Node & node){
        node.val = node.val* x + y;
    }
};
...
class SegmentTree{
    ...
    void updateIndex(int update_idx, int x, int y){
...
// THE IDEA IS THE SEGMENT TREE DOES NOT CHANGE AT ALL
// THE ONLY CHANGE HAS TO BE IN THE NODE :HOW WE COMBINE VALUES : EG MIN IN A RANGE
// HOW WE UPDATE THE GIVEN LEAF NODE


// RANGE : XOR of all values
// UPDATE : a[i] = a[i] * x + y
...
int main(){
    ...
    // 1 : update i to x  : 1 i x : A[I] = GCD(A[I], X)
    // 2 : give answer as RANGEMIN from index L to R : 2 L R
    int q;
    cin >> q;
    ...
    while(q--){
        ...
        if( type == 1){
            sg.updateIndex(i,x,y);
        }
        else {
            long long ans = sg.rangeQuery(i,x);
            cout << "Answer for [" << i << " : " << x << "] is "  <<   ans << endl;
        }
    }
